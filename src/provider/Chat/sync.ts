...
// This will be used later to generate embedding vectors for text queries.
...
export function useLinkSync() {
  ...
  useEffect(() => {
    // If no authenticated user, do not set up listener
    if (!authUser) return;

    // Create a Firestore query: get all links for the current user ordered by creation time (newest first)
    ...
    const unsubscribe = onSnapshot(linksQuery, async (snapshot) => {
      try {
        // Filter changes to only those that are 'added' or 'modified'
        ...
        // Map through each link and generate an embedding for the combined text fields
        const syncData = await Promise.all(links.map(async (link) => {
          // Concatenate various fields from the link into one string to generate the embedding
          const text = `${link.title || link.url} ${link.description || ""} ${link.tags?.join(" ") || ""} ${link.categories?.join(" ") || ""} ${link.searchTerms || ""}`;
          // Generate the embedding vector for the text query using our embeddings instance
          const vector = await embeddings.embedQuery(text);
          return {
            id: link.id,
            text,
            vector,
            // Prepare metadata as a JSON string, including various fields from the link
            metadata: JSON.stringify({
              ...
              userId: link.userId,
              // Convert Firestore timestamp to ISO string; if not available, use current date
              dateAdded: link.createdAt?.toDate().toISOString() || new Date().toISOString(),
            }),
          };
        }));

        // Send the generated sync data to the external service with retries
    ...
    // Clean up the snapshot listener when the component unmounts or authUser changes
    return () => unsubscribe();
  }, [authUser]);
}
